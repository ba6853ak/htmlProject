...
postForm.addEventListener("submit", function (e) {
    ...
    questions.forEach((questionElement, questionIndex) => {
        ...
        const options = Array.from(
            questionElement.querySelectorAll(".survey-option input")
...
<div id="merge">
  <button type="button" class="add-option-button button button-Third" data-question-id="${questionCount}">옵션 추가</button>
...
// 이벤트 위임을 통해 동적으로 추가된 옵션 삭제 및 추가 기능 구현
surveySection.addEventListener("click", function (event) {
    if (event.target && event.target.classList.contains("add-option-button")) {
        ...
        newOption.innerHTML = `
  <input type="text" name="surveyOptions${questionId}[]" placeholder="옵션 ${optionCount}을 입력하세요">
  <button type="button" class="remove-option">삭제</button>
`;
        optionsContainer.appendChild(newOption);
    } else if (event.target && event.target.classList.contains("remove-option")) {
        const option = event.target.closest(".survey-option");
        if (option) {
            option.parentElement.removeChild(option);
        }
    } else if (event.target && event.target.classList.contains("delete-question-button")) {
