...
function fetchUsers() {
    return fetch('http://localhost:3000/users')  // 서버에서 사용자 데이터를 가져오는 API
...
function renderTable(data) {
    ...
    data.forEach((user, index) => {
        ...
        row.innerHTML = `
              <td>${index + 1}</td>
            <td>${user.ID}</td>
            <td class="name-cell" data-id="${user.ID}" data-name="${user.Name}" data-birthday="${user.Birth}"
                data-gender="${genderText}" data-phone="${user.Phone}" data-email="${user.EMail}" 
                data-sc-id="${user.sc_id}"data-school="${user.schoolName}">${user.Name}</td> <!-- 여기에 SC_ID 추가 -->
            <td>${user.EMail}</td>
...
document.querySelectorAll('.role-select').forEach(select => {
    select.addEventListener('change', function () {
        const userId = this.closest('tr').querySelector('td:nth-child(2)').textContent;  // 수정된 부분
        const newRole = this.value;

        // 서버로 권한 변경 요청 전송
        fetch(`http://localhost:3000/users/${userId}/role`, {
            method: 'PUT',
            headers: {
...
document.getElementById('user-table-body').addEventListener('click', function (event) {
    if (event.target.classList.contains('delete-btn')) {  // 클릭한 요소가 delete-btn 클래스인지 확인
...
document.getElementById('user-table-body').addEventListener('click', function (event) {
    if (event.target.classList.contains('reset-btn')) {  // 클릭한 요소가 reset-btn 클래스인지 확인
        const row = event.target.closest('tr');
        ...
        // 사용자에게 확인 메시지 출력
        if (confirm(`비밀번호를 재설정하시겠습니까? 새로운 비밀번호는 "${newPassword}" 입니다.`)) {
            // 서버로 새로운 비밀번호 전송
            fetch(`http://localhost:3000/users/${userId}/reset-password`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
